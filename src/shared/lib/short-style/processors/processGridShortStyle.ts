...
export type GridShortStyle = Pu<{
  ...
  row: boolean // { gridAutoFlow: 'column' }
  column: boolean // { gridAutoFlow: 'row' }
  autoFlow: string // { gridAutoFlow }
  ...
  centerStart: boolean // true => { placeItems: 'center start' }
...
export const processGridShortStyle = <P extends object>(
  ...
  const gridCss = {
    ...
    ...row && { gridAutoFlow: 'column' },
    ...column && { gridAutoFlow: 'row' },
    ...isdef(autoFlow) && { gridAutoFlow: autoFlow },
